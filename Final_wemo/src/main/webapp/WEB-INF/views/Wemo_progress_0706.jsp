    ...
    $(function(){
        ...
        if (index != 0 && index != 8){
            ...
            function newMemoAppend(){
               ...
               success : function(rdata){// rdata == number 가져와서 입력해 줘야함 안쪽에 메모 생성 메서드 필요함
                ...
                var newMemobox = "<div calss='section section-"+sub+"'><form><div class = 'container memobox shadow-sm' style =' "+style+"'>"
                    + "<div class = 'container memo-top'><span class = 'date'>" + today
                    + "<span class='memo-num'>"+ rdata.MEMO_NUM +"</span>"
                    + "<span class = 'material-icons delete float-right'>delete</span>"
                    + "<span class = 'material-icons favorites float-right'>stars</span>"
                    + "<span class = 'material-icons float-right lock'>lock_open</span></div>"
                    + recommendContainer
                    + "<textarea class = 'memotext form-control' style ='overflow-y:hidden; resize:none; background-color:khaki; border:none;'></textarea></div>";
